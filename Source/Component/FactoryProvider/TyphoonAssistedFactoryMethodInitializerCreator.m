...
// TODO: break this file in several
...
- (NSArray *)prepareArgumentsWithValues:(void **)arguments into:(void **)output
{
    ...
    *instancePointer = (__bridge void *)instance;
    ...
    [_parameters enumerateObjectsUsingBlock:^(id parameter, NSUInteger idx, BOOL *stop) {
        ...
        if ([parameter isKindOfClass:[TyphoonAssistedFactoryParameterInjectedWithArgumentIndex class]]) {
            output[idx + 2] = arguments[[parameter argumentIndex] + 2];
            ...
            NSString *selector = [NSString stringWithCString:sel_getName([parameter property]) encoding:NSASCIIStringEncoding];
            ...
            *valuePointer = (__bridge void *)value;
            ...
            output[idx + 2] = valuePointer;
        } else {
            NSAssert(NO, @"Unknown parameter type %@", [parameter className]);
        }
    }];
...
- (ffi_type **)typesWithEncodeString:(const char *)encodeString count:(unsigned int *)count
{
    ...
    // Will be freed on dealloc
    ffi_type **argTypes = calloc(argCount, sizeof(ffi_type *));
...
- (void)createFromProtocol:(Protocol *)protocol inClass:(Class)factoryClass
{
    ...
    NSString *key = [NSString stringWithFormat:@"%s %s",
                     ...
                     sel_getName(self.factoryMethod.selector)];
