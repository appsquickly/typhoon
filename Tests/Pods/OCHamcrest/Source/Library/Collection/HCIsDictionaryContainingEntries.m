...
- (BOOL)matches:(id)dict describingMismatchTo:(id <HCDescription>)mismatchDescription
{
    ...
    for (NSUInteger index = 0; index < count; ++index) {
        id key = keys[index];
        if (dict[key] == nil) {
            [[[[mismatchDescription appendText:@"no "] appendDescriptionOf:key] appendText:@" key in "] appendDescriptionOf:dict];
            return NO;
        }
        ...
        if (![valueMatcher matches:actualValue]) {
            [[[[mismatchDescription appendText:@"value for "] appendDescriptionOf:key] appendText:@" was "]
...
{
    [[[[description appendDescriptionOf:keys[index]] appendText:@" = "] appendDescriptionOf:valueMatchers[index]] appendText:@"; "];
}
...
static void requirePairedObject(id obj) {
    if (obj == nil) {
        @throw [NSException exceptionWithName:@"NilObject" reason:@"HC_hasEntries keys and value matchers must be paired" userInfo:nil];
    }
}
...
id <HCMatcher> HC_hasEntries(id keysAndValueMatch, ...) {
    ...
    return [HCIsDictionaryContainingEntries isDictionaryContainingKeys:keys valueMatchers:valueMatchers];
}
